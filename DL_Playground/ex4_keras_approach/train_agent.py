...
while nepisodes < N_EPISODES_TOTAL_TRAIN:
    if state.terminal or epi_step >= opt.early_stop or episode_reward < -10:
        ...
        if nepisodes%1==0:
            print("Episode {}/{}\t|Epsilon: {:.2f}\t|e-greedy ratio: {:.2f}\t episode reward: {}".format(nepisodes+1,N_EPISODES_TOTAL_TRAIN,agent.epsilon,agent.greedy/agent.rnd,episode_reward))
